#!/usr/bin/env python3
"""
actualizar_stock.py — Actualiza el stock de un producto base en la KB de Palishopping.

Uso: python3 actualizar_stock.py
"""
...
def mostrar_bienvenida():
    console.print()
    console.print(Panel(
        "[bold cyan]Actualizar Stock — Palishopping KB[/bold cyan]\n"
        "[dim]Modifica el stock de un producto base existente.[/dim]",
        box=box.ROUNDED,
        padding=(0, 2),
    ))
    console.print()
...
def mostrar_tabla(catalogo: list) -> dict:
    ...
    table.add_column("Stock",  style="bold white",  width=8,  justify="right")

    indice = {}
    for i, entrada in enumerate(catalogo, 1):
        stock_str = str(entrada.get("stock", "—"))
        table.add_row(str(i), entrada["sku"], entrada["nombre"], stock_str)
...
def elegir_producto(catalogo: list) -> dict | None:
    """Devuelve la entrada del catálogo elegida o None si cancela."""
    if not catalogo:
...
def pedir_nuevo_stock(sku: str, stock_actual: int) -> int | None:
    """Muestra el stock actual y pide el nuevo valor. Retorna el nuevo stock o None si cancela."""
    console.print()
    console.print(f"  Stock actual de [bold yellow]{sku}[/bold yellow]: [bold]{stock_actual:,}[/bold] unidades")
    console.print()

    while True:
        valor = Prompt.ask(
            "[bold]Nuevo stock[/bold] [dim](Enter para cancelar)[/dim]",
            default="",
        )
        ...
        try:
            nuevo = int(valor.strip())
            if nuevo < 0:
                console.print("[red]El stock no puede ser negativo.[/red]")
                continue
            return nuevo
        except ValueError:
            console.print("[red]Ingresá un número entero.[/red]")


def mostrar_confirmacion(sku: str, stock_viejo: int, stock_nuevo: int) -> bool:
    table = Table(box=box.SIMPLE, show_header=False, padding=(0, 2))
    table.add_column("Campo", style="bold cyan", min_width=16)
    table.add_column("Valor")
    table.add_row("SKU",          f"[bold yellow]{sku}[/bold yellow]")
    table.add_row("Stock actual", f"[dim]{stock_viejo:,}[/dim]")
    table.add_row("Nuevo stock",  f"[bold green]{stock_nuevo:,}[/bold green]")
...
def actualizar(sku: str, stock_nuevo: int):
...
def mostrar_resultado(sku: str, stock_viejo: int, stock_nuevo: int):
    ...
    console.print()
    console.print(Panel(
        f"[bold green]Stock actualizado.[/bold green]\n"
        f"[bold yellow]{sku}[/bold yellow]: "
        f"[dim]{stock_viejo:,}[/dim] → [bold]{stock_nuevo:,}[/bold]  "
        f"[{color}]({signo}{diferencia:,})[/{color}]",
        box=box.ROUNDED,
        padding=(0, 2),
    ))
    console.print()
...
def main():
    ...
    try:
        ...
        sku = entrada["sku"]
        stock_actual = entrada.get("stock", 0)

        console.rule("[bold cyan]Actualizar stock")
        stock_nuevo = pedir_nuevo_stock(sku, stock_actual)

        if stock_nuevo is None:
            console.print("\n[yellow]Cancelado.[/yellow]\n")
            sys.exit(0)

        if not mostrar_confirmacion(sku, stock_actual, stock_nuevo):
            console.print("[yellow]Cancelado. No se realizaron cambios.[/yellow]\n")
            sys.exit(0)

        actualizar(sku, stock_nuevo)
        mostrar_resultado(sku, stock_actual, stock_nuevo)
